@extends('backend.layouts.app')

@section('title', 'Registration')

@section('content')
    <section>
        <div class="section-body">
            <div class="card">
                <div class="card-head">
                    <header class="text-capitalize">all registrations</header>
                    <div class="tools">
                        <a class="btn btn-primary ink-reaction" href="{{ route('registration.create') }}">
                            <i class="md md-add"></i>
                            Add
                        </a>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover" id="registration-datatable">
                            <thead>
                            <tr>
                                <th>{{ strtoupper(__('name')) }}</th>
                                <th>{{ strtoupper(__('user type')) }}</th>
                                <th>{{ strtoupper(__('address')) }}</th>
                                <th>{{ strtoupper(__('phone no')) }}</th>
                                <th>{{ strtoupper(__('email')) }}</th>
                                <th>{{ strtoupper(__('age')) }}</th>
                                <th>{{ strtoupper(__('gender')) }}</th>
                                <th>{{ strtoupper(__('dob')) }}</th>
                                {{-- <th>{{ strtoupper(__('marital status')) }}</th> --}}
                                <th>{{ strtoupper(__('action')) }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </section>
@stop

@push('styles')
<link rel="stylesheet" type="text/css" href="{{ asset('vendor/datatables/datatables.min.css') }}">
@endpush

@push('scripts')
<script src="{{ asset('vendor/datatables/datatables.min.js') }}"></script>
<script>
    $(document).ready(function () {
        var table = $('#registration-datatable').DataTable({
            "dom": "rBftip",
            "language": {
                "processing": "<h2 id='dt_loading'><span class='fa fa - spinner fa-pulse'></span> Loading...</h2>"
            },
            "buttons": [
                'pageLength', 'colvis'
            ],
            "order": [],
            "processing": true,
            "serverSide": true,
            "ajax": {
                "type": "POST",
                "data": {"_token": '{{ csrf_token() }}'},
                "url": '{{ route('registration.datatable') }}'
            },
            "pageLength": "25",
            "columns": [
                {"data": "name", "name": "name", "searchable": "false"},
                {"data": "user_type", "name": "user_type"},
                {"data": "address", "name": "address"},
                {"data": "phone", "name": "phone"},
                {"data": "email", "name": "email"},
                {"data": "age", "name": "age"},
                {"data": "gender", "name": "gender"},
                {"data": "dob", "name": "dob"},
                // {"data": "marital_status", "name": "marital_status"},
                {
                    "data": "username", "class": "text-right", "orderable": false, "render": function (data) {
                    return "<a href='/registration/" + data + "/edit' class='btn btn-default'> Edit </a>" +
                        "<button data-url='/registration/" + data + "/destroy' class='btn btn-danger item-delete'> Delete </button>";
                }
                }
            ]
        });

        $(document).on('click', '.uk-button-delete', function () {
            var that = this;
            var name = $(that).data('name');
            UIkit.modal.confirm("Delete this '" + name + "' ?").then(function () {
                $(that).closest('form').submit();
            });
        });
        $('input[type=search]').addClass('uk-input');
    });
</script>
@endpush